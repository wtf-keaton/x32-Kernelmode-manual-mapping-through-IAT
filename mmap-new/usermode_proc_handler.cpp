...
uint32_t usermode_proc_handler::get_module_base(const std::string &module_name) {
	...
	if (Module32First(snapshot, &module_entry)) {
		do {
			if (!_stricmp(module_entry.szModule, module_name.c_str())) {
				...
				return ( uint32_t )module_entry.hModule;
			}
			module_entry.dwSize = sizeof(MODULEENTRY32);
...
uint32_t usermode_proc_handler::virtual_protect( uint32_t address, size_t size, uint32_t protect) {
	DWORD old_protect{};
	VirtualProtectEx(handle, (LPVOID)address, size, protect, &old_protect);
	return old_protect;
}

uint32_t usermode_proc_handler::virtual_alloc(size_t size, uint32_t allocation_type, uint32_t protect, uint32_t address) {
	return ( uint32_t )VirtualAllocEx(handle, (void*)address, size, allocation_type, protect);
}