...
public:
	...
	virtual	uint32_t get_module_base(const std::string &module_name) override;
	...
	virtual uint32_t virtual_protect( uint32_t address, size_t size, uint32_t protect) override;

	virtual uint32_t virtual_alloc(size_t size, uint32_t allocation_type, uint32_t protect, uint32_t address = NULL) override;
};