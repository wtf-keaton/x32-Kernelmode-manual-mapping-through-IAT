...
uint32_t kernelmode_proc_handler::get_module_base(const std::string &module_name) {
	if (handle == INVALID_HANDLE_VALUE)
		return 0;
...
uint32_t kernelmode_proc_handler::virtual_protect( uint32_t address, size_t size, uint32_t protect) {
	if (handle == INVALID_HANDLE_VALUE)
		return 0;
	DWORD bytes_read;
...
uint32_t kernelmode_proc_handler::virtual_alloc(size_t size, uint32_t allocation_type, uint32_t protect, uint32_t address) {
	if (handle == INVALID_HANDLE_VALUE)
		return 0;
	DWORD bytes_read;
